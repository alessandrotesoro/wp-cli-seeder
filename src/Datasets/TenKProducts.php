...
use Sematico\Seeder\Traits\CanUploadImages;
use WP_CLI;

/**
 * Generate a dataset of 10,000 products with images.
 */
class TenKProducts extends BaseDataset {
	...
	public function __construct() {
		$this->set_name( '10k products with images' );
	}
	...
	public function generate() {
		...
		$how_many_products = $this->ask_number( 'How many products do you want to generate?' );

		if ( $how_many_products > 10000 ) {
			WP_CLI::error( 'You can generate up to 10,000 products.' );
		}

		$this->items_to_seed = $how_many_products;

		$generate_images = $this->ask_images();

		if ( $generate_images ) {
			$this->skip_images = false;
		} else {
			$this->skip_images = true;
		}
